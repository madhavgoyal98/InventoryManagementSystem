...
<body style="background-color:transparent; margin-top: 8%; margin-left: 5%">
		...
		<form action="<?php echo('update_item.php?id='.$id.''); ?>" method="post">
			<table class='table table-hover table-responsive table-bordered' style="width: 100%">
				...
				<tr>
					...
					<td style="width: 40%;"><input type='text' name='vendor' class='form-control' value='<?php echo($order->vendor); ?>' maxlength="50" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' readonly></td>
				</tr>

				<tr>
					<td style="width: 30%;">Start Date</td>
					<td><input type="date" name='sdate' class='form-control' value='<?php echo($order->start_date); ?>' readonly></td>
				</tr>
				
				<tr>
					<td style="width: 30%;">End Date</td>
					<td><input type="date" name='edate' class='form-control' value='<?php echo($order->end_date); ?>' readonly></td>
				</tr>
	...
	<?php
		...
		echo("<div class='right-button-margin'>");
			echo("<a href='worker_orders.php' class='btn btn-default pull-right'>Back</a>");
		echo("</div>");
