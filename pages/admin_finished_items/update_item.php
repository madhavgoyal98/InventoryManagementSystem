...
<body style="background-color:transparent; margin-top: 8%; margin-left: 5%">
		...
		<form action="<?php echo('update_item.php?id='.$id.''); ?>" method="post">
			<table class='table table-hover table-responsive table-bordered' style="width: 100%">
				...
				<tr>
					...
					<td><input type='text' name='name' value='<?php echo($fp->name); ?>' class='form-control' maxlength="100" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' required></td>
				</tr>
				...
				<tr>
					...
					<td><input type='text' name='measuring' value='<?php echo($fp->measuring_unit); ?>' class='form-control' maxlength="20" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' required></td>
				</tr>
	...
	<?php
		...
		echo("<div class='right-button-margin'>");
			echo("<a href='admin_finished_items.php' class='btn btn-default pull-right'>Back</a>");
		echo("</div>");
