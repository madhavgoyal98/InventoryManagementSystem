		...
		<form action="<?php echo('update_user.php?id='.$id.''); ?>" method="post">
			<table class='table table-hover table-responsive table-bordered' style="width: 100%">
				...
				<tr>
					...
					<td><input type='text' name='name' value='<?php echo($user_details[0]) ?>' class='form-control' maxlength="40" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' required></td>
				</tr>

				<tr>
					<td style="width: 30%">Password</td>
					<td><input type='password' name='password' value='' class='form-control' maxlength="30" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' required></td>
				</tr>
