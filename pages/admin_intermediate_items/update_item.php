...
<body style="background-color:transparent; margin-top: 8%; margin-left: 5%">
		...
		<form action="<?php echo('update_item.php?id='.$id.''); ?>" method="post">
			<table class='table table-hover table-responsive table-bordered' style="width: 100%">
				...
				<tr>
					...
					<td><input type='number' name='quantity' value='<?php echo($im->quantity); ?>' class='form-control' maxlength="11" min="0" oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' required></td>
				</tr>
	...
	<?php
		...
		echo("<div class='right-button-margin'>");
			echo("<a href='admin_intermediate_items.php' class='btn btn-default pull-right'>Back</a>");
		echo("</div>");
